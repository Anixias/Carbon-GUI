...
public class TreeListItem : MarginContainer
{
    ...
    public override void _Process(float delta)
    {
        ...
        if (!GetGlobalRect().HasPoint(pos))
        {
            dragging = false;
            DragLocation = DragDropLocation.none;
            WarpMouse(GetLocalMousePosition());
            return;
        }
    ...
    {
        if (lockButton == null) return false;
        if (!lockButton.IsVisibleInTree()) return false;
    ...
    {
        if (dropArrow == null) return false;
        if (!dropArrow.IsVisibleInTree()) return false;
    ...
    public bool can_drop_data_fw(Vector2 position, object data, Control source)
    {
        ...
        {
            dragging = true;
            if (treeListItem == this) return false;
            if (treeListItem.ContainsChildItem(this)) return false;
            ...
            if (treeListItem.ContainingTreeList == containingTreeList)
            {
                ...
                if (canContainItems)
                {
                    ...
                    else DragLocation = DragDropLocation.within;
                }
    ...
    {
        if (ContainsChildItem(parent)) return;
    ...
    {
        if (!Godot.Object.IsInstanceValid(listItem)) return;
